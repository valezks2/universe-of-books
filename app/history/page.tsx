...
import { Book, MessageSquare, Calendar, ChevronRight, Search } from "lucide-react";
  ...
  const fetchHistory = async () => {
    ...
    if (error) {
      ...
      const formattedData = data.map((chat: any) => ({
        ...
        recommendationsCount: chat.chat_messages?.filter(
          (m: any) => m.books_metadata && Object.keys(m.books_metadata).length > 0
        ).length || 0,
      }));
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="space-y-4">
          ...
          {!loading && filteredHistory.map((item) => (
            <div 
              key={item.id}
              className="group flex items-center justify-between p-5 bg-white border border-[#ededed] rounded-2xl transition hover:border-[#ccc] hover:shadow-sm cursor-pointer"
            >
              <div className="flex items-start gap-4">
                <div className="mt-1 bg-[#f5f5f5] p-2.5 rounded-full text-[#555] group-hover:bg-[#e7e7e7] transition flex-shrink-0">
                  <MessageSquare size={18} />
                </div>
                <div>
                  <h3 className="text-[1em] font-medium text-[#222] leading-snug mb-1">
                    "{item.title}"
                  </h3>
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[0.85em] text-[#888]">
              ...
              <div className="text-[#999] group-hover:text-[#222] transition flex-shrink-0 ml-2">
